...
function displayProduct() {
  ...
  productlist.map(product => {
    ...
    productBox.className = "products-box";
...
let cartItem = JSON.parse(localStorage.getItem("store")) || [];
...
function displayCart() {
  ...
  cartItem.map((cart, index) => {
   sum+=cart.price*cart.qty;
    ...
    qty.textContent="QTY:"+cart.qty
    button.textContent = "Delete";
    
    // it is create for delete selected item from cart
    button.addEventListener("click", () => {
      clearCart(index);
    })
   ...
   // for billing
    ...
    // give name of billing div and other element 
    let discounts=(sum*5/100).toFixed(2);
    totalDiv.className="total-div";
    amount.textContent="Amount:"+sum.toFixed(2);
    discount.textContent="Discount(5%):"+discounts;
    total.textContent="Total Amount:"+(sum-discounts).toFixed(2);

    // now all element are send in total-div through appendchild
    totalDiv.appendChild(amount);
    totalDiv.appendChild(discount);
...
let existItemInCart=false;
 ...
 if(existItemInCart==false){
  ...
  localStorage.setItem("store", JSON.stringify(cartItem));
  displayCart();
}
